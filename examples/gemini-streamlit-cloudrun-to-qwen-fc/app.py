...
"""
This module demonstrates the usage of the Qwen API in Alibaba Cloud Model Studio
within a Streamlit application.

Migrated from the Google Cloud "gemini-streamlit-cloudrun" sample by CloudPort Agent.
Original: https://github.com/GoogleCloudPlatform/generative-ai/tree/main/gemini/sample-apps/gemini-streamlit-cloudrun

Migration strategy: provider-specific differences are localized in
`cloudport_compat.py`, which mirrors the small `google-genai` surface used by
the original sample (Part / GenerateContentConfig / ThinkingConfig /
client.models.generate_content). The app body stays close to upstream while
fixing two upstream multimodal UI bugs documented in MIGRATION_NOTES.md.
"""
...
# ---------------------------------------------------------------------------
# Current source-side reference tiers (verified 2026-07-11)
#
# The upstream Google sample still lists Gemini 2.0/2.5 IDs. For migration
# guidance, normalize the source model picker to current task tiers instead:
#
#   gemini-3.1-pro-preview  -> qwen3.7-plus  (high-quality multimodal)
#   gemini-3.5-flash        -> qwen3.6-plus  (balanced multimodal)
#   gemini-3.1-flash-lite   -> qwen3.6-flash (low-cost multimodal)
#
# This is a task-profile mapping, not a claim of exact capability parity.
# All target choices below accept text, image, and video inputs so one global
# model selector can safely drive every tab in this sample.
# ---------------------------------------------------------------------------
MODELS = {
...
# These hybrid models support enable_thinking/thinking_budget and currently
# enable thinking by default. The UI can keep the model default, cap the
# reasoning budget, or explicitly disable thinking.
THINKING_BUDGET_MODELS = set(MODELS)


@st.cache_resource
def load_client() -> QwenClient:
    """Load the Model Studio OpenAI-compatible client."""
    ...
    if not api_key:
        st.error(
            "🚨 Configuration Error: set `DASHSCOPE_API_KEY`. "
            "For production, also set `DASHSCOPE_BASE_URL` to the "
            "workspace-scoped endpoint for the same region as the API key."
        )
        st.stop()

    return QwenClient(api_key=api_key, base_url=base_url, timeout=timeout)
...
def get_model_name(name: str | None) -> str:
    ...
    if not name:
        return "Qwen"
    return MODELS.get(name, "Qwen")


QWEN_CHAT_URL = "https://chat.qwen.ai/"


def generate_qwen_response(*, model, contents, config=None):
    """Call Model Studio and render a Qwen-branded error instead of crashing."""
    ...
    except Exception as exc:  # API/transport errors should remain visible in the demo.
        message = str(exc)
        st.error("Qwen API request failed.")

        if (
            "max_completion_tokens" in message
            and "thinking_budget" in message
        ):
            st.warning(
                "The total output limit must be larger than the thinking budget. "
                "This build normally adjusts it automatically; restart Streamlit "
                "after replacing both app.py and cloudport_compat.py."
            )

        with st.expander("Error details"):
            st.code(message, language="text")
        st.link_button("Ask Qwen", QWEN_CHAT_URL)
        return None
...
fc_function_name = os.environ.get("FC_FUNCTION_NAME")
if fc_function_name:
    st.link_button(
        "Open in Function Compute",
        "https://fcnext.console.aliyun.com/",
    )

st.header(":sparkles: Qwen API in Alibaba Cloud Model Studio", divider="rainbow")
client = load_client()
...
if selected_model in THINKING_BUDGET_MODELS:
    ...
    if thinking_budget_mode == "Manual":
        thinking_budget = st.slider(
            ...
            help="The accepted maximum is model-specific; lower this if the API rejects it.",
        )
...
with freeform_tab:
    ...
    temperature = st.slider(
        ...
        min_value=0.0,
        max_value=1.9,  # Qwen accepts [0, 2) exclusive; Gemini allowed up to 2.0
        value=0.5,
        step=0.05,
        key="temperature",
    )
    ...
    if thinking_budget is not None and thinking_budget > 0:
        effective_total_tokens = compute_max_completion_tokens(
            max_output_tokens, thinking_budget
        )
        st.caption(
            "Manual thinking: API total output limit "
            f"`max_completion_tokens={effective_total_tokens}` "
            f"(answer {max_output_tokens} + thinking {thinking_budget} + safety margin)."
        )
    ...
    if generate_freeform and prompt:
        with st.spinner(
            ...
            with first_tab2:
                st.markdown(
                    f"""Parameters:\n- Model ID: `{selected_model}`\n- Temperature: `{temperature}`\n- Top P: `{top_p}`\n- Max Answer Tokens: `{max_output_tokens}`\n"""
                )
                if thinking_budget is not None:
...
with tab3:
    ...
    vision_config = GenerateContentConfig(
        temperature=0.2,
    ...
    with furniture:
        st.markdown(
            """In this demo, you will be presented with a scene (e.g., a living room) and will use the Qwen model to perform visual understanding. You will see how Qwen can be used to recommend an item (e.g., a chair) from a list of furniture options as input. You can use Qwen to recommend a chair that would complement the given scene and will be provided with its rationale for such selections from the provided list."""
        )
    ...
    with oven:
        ...
        st.write(
            "Equipped with the ability to extract information from visual elements on screens, Qwen can analyze screenshots, icons, and layouts to provide a holistic understanding of the depicted scene."
        )
...
"""
    ...
    with er_diagrams:
        ...
        st.write(
            "Qwen multimodal capabilities empower it to comprehend diagrams and take actionable steps, such as optimization or code generation. The following example demonstrates how Qwen can decipher an Entity Relationship (ER) diagram."
        )
    ...
    with glasses:
        ...
        st.write(
            """Qwen is capable of image comparison and providing recommendations. This can be useful in industries like e-commerce and retail.
    ...
    with math_reasoning:
        ...
        st.write(
            "Qwen can also recognize math formulas and equations and extract specific information from them. This capability is particularly useful for generating explanations for math problems, as shown below."
        )
...
with tab4:
    ...
    video_config = GenerateContentConfig(
        temperature=0.2,
        max_output_tokens=4096,
        thinking_config=thinking_config,
    )
    ...
    with vide_desc:
        st.markdown(
            """Qwen can also provide the description of what is going on in the video:"""
        )
    ...
    with video_tags:
        st.markdown(
            """Qwen can also extract tags throughout a video, as shown below:."""
        )
    ...
    with video_highlights:
        st.markdown(
            """Below is another example of using Qwen to ask questions about objects, people or the context, as shown in the video about Pixel 8 below:"""
        )
...
Provide the answer in table format.
    ...
    with video_geolocation:
        st.markdown(
            """Even in short, detail-packed videos, Qwen can identify the locations."""
        )
